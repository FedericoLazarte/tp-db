create or replace function apertura_funcion(_id_sala int, _fecha_inicio timestamp, _id_pelicula int)
returns int as $$
declare
    _id_funcion int := -1;
    _fecha_fin date;
...
begin
    ...
    if not (_fecha_inicio > now()) then
        insert into error (
            ...
            'apertura_funcion', _id_sala, _fecha_inicio,
            _id_pelicula, null, null, null, null,
            now(), 'no se permite abrir una nueva función con retroactividad'
        );
        return _id_funcion;
    end if;
