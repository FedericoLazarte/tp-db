...
begin
    -- id funcion existe?
    select id_sala, butacas_disponibles 
    ...
    if not found then
        ...
        values ('compra butaca', _nro_fila, _nro_butaca, _id_cliente, now(), '?id de función no válido.');
        return false;
    end if;

    -- rango valido de nro fila o butaca
    ...
    -- nro de fila y butaca reservada o comprada?
    ...
    if found and v_estado in ('reservada', 'comprada') and v_cliente_existente != _id_cliente then
